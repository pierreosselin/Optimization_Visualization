...
function test_sgdM(h, eps, nlim) {
  ...
  var x_ini = [1.5];
  var objective = square;
  var alg = new GradientDescentMomentum(objective,x_ini, h, 0.8);
  alg.optimize(eps, nlim);
  ...
  function onFunctionChanged(myfunction) {
    ...
    // Change Alg and make optimizization
    alg = new GradientDescentMomentum(objective, x_ini, h, objectives[myfunction].delta.value);
    alg.optimize(eps, nlim);
    graphPlot = new GraphPlot(
      d3.select('#svg1'),
      objectives[myfunction].xDomain,
