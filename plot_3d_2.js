...
const x = d3.scaleLinear([-2, 3], [0, width + 28]);
const y = d3.scaleLinear([-2, 3], [height, 0]);
...
const svg = d3.select('#svg1').attr("viewBox", [0, 0, width + 28, height])
    .style("display", "block")
    .style("margin", "0 -14px")
    .style("width", "calc(100% + 28px)");


const thresholds = d3.range(0, 20).map(i => Math.pow(2, i));

const color = d3.scaleSequentialLog(d3.extent(thresholds), d3.interpolateYlGnBu);
...
function _makeGrid() {
    ...
    for (let j = 0; j < m; ++j) {
        for (let i = 0; i < n; ++i) {
            grid[j * n + i] = rosenbrock(x.invert(i * q + x0), y.invert(j * q + y0));
        }
    }
    grid.x = -q;
    grid.y = -q;
